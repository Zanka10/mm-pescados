    ...
    <>
      ...
      <div className="card">
        <div className="table users">
          ...
          {current.map((u, idx) => (
            <div className="table-row" key={u.email + idx}>
              <div className="td">
                <div className="td-title">{u.name}</div>
              </div>
              <div className="td">{u.email}</div>
              <div className="td">{u.cpf}</div>
              <div className="td">{u.phone}</div>
              <div className="td">{u.role}</div>
              <div className="td">
                <span className={`status-chip ${u.status === 'Ativo' ? 'chip-done' : 'chip-cancel'}`}>{u.status}</span>
              </div>
        ...
        <div className="table-footer">
          <div>
            Mostrando {Math.min((page - 1) * pageSize + 1, filtered.length)} até {Math.min(page * pageSize, filtered.length)} de {filtered.length} usuários
          </div>
          <div className="pager">
